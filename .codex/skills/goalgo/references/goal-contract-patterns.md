...
A Goal is a thread-scoped completion contract. It keeps Codex working toward a defined outcome across turns and requires concrete evidence before completion.

Use Goals when:

- The finish line is clear but the path is uncertain.
- The next action depends on what Codex learns after tests, logs, code inspection, benchmarks, generated artifacts, or research evidence.
...
| Iteration policy | Guides continuation | After each attempt, how should Codex choose the next action? |
| Blocked stop condition | Defines honest stopping | When should Codex stop and what should it report? |
...
Keep the internal `goal_body` without the `/goal` prefix. Add the prefix only to the copyable display version.

## Confirmation and Launch Cases

| Situation | Required behavior |
|---|---|
| User asks only for a prompt | Produce the copyable `/goal`; never call `create_goal`. |
| User says “看起来不错” | Treat it as feedback, not launch authorization. |
| User confirms the unique latest vN | Call `get_goal`, then create only if no unfinished conflict exists. |
| User changes the draft | Increment to vN+1, show the full new text, and require confirmation again. |
| Same objective is already active | Do not create a duplicate; continue the existing Goal. |
| A different unfinished Goal exists | Do not overwrite or mark it complete; ask the user to resolve it or use another thread. |
| Same Goal is paused or blocked | Do not assume permission to resume or replace it; report the state and required user action. |
| `get_goal` fails | Do not attempt a blind create. |
| `create_goal` is unavailable or fails | Label the Goal “未启动” and provide the copyable `/goal`. |
| User did not specify a token budget | Omit `token_budget`. |
| Goal contains destructive or external actions | Starting the Goal does not waive the normal approval boundary. |
| Goal creation succeeds | Verify the state and begin the first safe, in-scope action immediately. |
...
- Design-only mode makes zero Goal creation calls.
- Editing v1 invalidates its confirmation and produces v2.
- Confirming vN performs `get_goal` before at most one `create_goal` call.
- An unfinished conflicting Goal produces an honest conflict report and no replacement call.
- A missing tool produces a manual fallback labeled “未启动”.
