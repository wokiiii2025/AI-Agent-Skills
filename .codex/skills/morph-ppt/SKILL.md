...
- Grid math - 33.87  19.05cm, edge margin  1.27cm, inter-block gap  0.76cm,  20% negative space. For N-card grids: `col = (33.87  2margin  (N1)gap) / N`.
...
- **5b-morph-4 - `#sN-*` lingering on slide N+1 (ghost leak).** Per-slide content MUST be ghosted (`x=36cm`) on the NEXT slide. Loop + filter per N2:
  ```bash
  NSLIDES=$(officecli query "$FILE" slide --json | jq '.data.results | length')
