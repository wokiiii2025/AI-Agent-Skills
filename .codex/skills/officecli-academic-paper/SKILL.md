...
officecli add "$FILE" /body --type paragraph --prop text="The Ming dynasty's  policy shaped coastal trade for two centuries." --prop size=12pt --prop lineSpacing=1.5x --prop firstLineIndent=720
...
officecli add "$FILE" /body --type paragraph --prop text="Ming Shilu . Taipei: Academia Sinica, 1966." --prop size=12pt --prop indent=720 --prop hangingIndent=720
...
```bash
officecli view "$FILE" text | head -20       #  + , , x must appear as unicode math (verified renders)
...
> (d) Do equations render as math? (Italicized variables, Greek letters like  / , proper integrals / fractions - NOT plain-text `lambda_1`, `x_{t+1}`, `\int`.)
