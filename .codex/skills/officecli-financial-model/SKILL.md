...
When the ask contains "print" / "" / "" / "" / "IC memo" / "LP update", the print pipeline must emit **only** the Outputs zone. Two artefacts:
...
**Step 5 - Cash Flow rows (all formulas).** Operating: `NI + D&A  WorkingCapital`. Investing: `CapEx`. Financing: `Debt  Dividends`. Ending Cash = `Opening + Operating + Investing + Financing`. **Year 2+ Opening Cash = prior period Ending Cash** - self-chain on the same sheet: `C17 = B19`, `D17 = C19`, `E17 = D19`. The Y1 `OpeningCash` is an Assumptions input.
...
**Step 2 - FCF build (10 years).** Columns B:K = Y1..Y10. Rows: `Revenue` (from growth) / `EBIT` (revenue  margin) / `EBIT  (1  TaxRate)` (NOPAT) / `+ D&A` / ` CapEx` / ` NWC` / `= FCF`. Use Assumptions-driven ratios (`CapEx = Revenue  CapExRatio`). All cells formulas, black font, `numFmt='$#,##0;($#,##0);"-"'`.

**Step 3 - WACC panel.** On `WACC` sheet, an 8-row panel: `Risk-free rate` / `Equity risk premium` / `Beta` / `Cost of equity` (=Rf + ERP) / `Pre-tax debt cost` / `After-tax debt cost` (=(1TaxRate)) / `Equity weight` / `Debt weight` / `WACC` (=WeRe + WdRd_after_tax). Inputs blue; derived rows black.
...
**Step 5 - 2-axis sensitivity grid (WACC  g).** 55 grid. Rows = WACC values `7.5% ... 11.5%`, cols = `g` values `1.5% ... 3.5%`. Each cell = one self-contained formula re-running the DCF with the grid's WACC and g substituted. Template:
...
Copy the formula across D15:H19 (55 grid). Row 14 carries g values (blue input); column C carries WACC values (blue input). Row 13 and column B carry labels. Apply 3-color gradient CF for quick-read (green = upside, red = downside):
...
python3 -c "import sys; sys.exit(0 if float('$DELTA') <= 1 else 1)" && echo "S&U sponsor OK (stated=$STATED plug=$PLUGGED)" || { echo "REJECT Gate 4 S&U: stated $STATED  plug $PLUGGED (=$DELTA) - fees silently absorbed"; exit 1; }
```
...
- **Visible text in an adjacent Notes column** -> `{"command":"set","path":"/DCF/D3","props":{"value":"TV = FCF  (1+g) / (WACCg)"}}` - **`value`, not `formula`**, plain quoted string.
...
- **(c) 2-axis sensitivity grid** - 55 or 77, one self-contained formula per cell, row/col headers are the two drivers. See Recipe B Step 5 for WACC  g.
...
**Football-field chart pattern (DCF valuation summary).** Horizontal Low->High bars for 3-5 valuation methods (DCF base, DCF bear, Trading comps, Precedent txns, LBO floor) stacked vertically. On a `Football` sheet: col A = method label, col B = Low $, col C = High $, col D = `=CB` (width). Chart as a stacked bar with column B as an invisible first series (white/no-fill) and column D as the visible series - `dataRange=Football!A3:D7`, `chartType=bar`. Excel reads this as a floating bar per method.
