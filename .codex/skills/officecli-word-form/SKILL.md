...
1. **Anchor must be globally unique.** In bilingual contracts "" matches both parties - use a unique phrase like "Service Provider" or full English title.
...
3. **Chinese + full-width parens ``** match literally in `find`, but when unsure, `officecli view "$FILE" text | grep -n ""` first to confirm the exact bytes in the file.

```bash
# Trap: first-match hits  only,  missed
officecli add "$FILE" /body --type sdt --after 'find:'

# Fix: two signatories, two unique anchors
officecli add "$FILE" /body --type sdt --prop alias=Party_A_Name --prop tag=party_a \
  --after 'find:Service Provider'
...
**Do not simulate fields with underscores.** `_______________` produces zero structured data and leaks past every verification. Always use `--type sdt` or `--type formfield`.
...
| date SDT with non-default format | **A** | `--prop format="yyyyMMdd"` |
...
officecli get "$FILE" '/body/sdt[N]'   # expect: type=date format=yyyyMMdd
```
